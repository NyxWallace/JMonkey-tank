...
public class PhysicsTankControl extends AbstractControl implements TankControl, PhysicsListener, Cloneable, CollisionListener {
	...
	protected void controlUpdate(float tpf) {
		...
		if (firing && lastTimeFired >= reloadTime) {
			...
			RigidBodyControl projectileRigidBodyControl = projectile.getControl(RigidBodyControl.class);
			if (projectileRigidBodyControl != null) {
				Vector3f linearVelocity = rigidBodyControl.getLinearVelocity();
				projectileRigidBodyControl.setLinearVelocity(linearVelocity);
			}
		...
		if (healthObject instanceof Float) {
			...
			if (health <= 0.0f) {
				...
				SpawnControl sc = new SpawnControl();
                                sc.setModel("Models/DeadTankModel.j3o");
                                Spatial deadTank = sc.spawn("ded");
                                DeadTankControl dtc = deadTank.getControl(DeadTankControl.class);
                                dtc.setOriginalTank((Node)tank.clone());
                                tank.getParent().attachChild(deadTank);
    ...
    public void collision(Spatial spatial, Vector3f point, Vector3f normal) {
        //System.out.println(tank.getUserData("health"));
    }
}
