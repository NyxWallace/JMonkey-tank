...
public class KinematicTankControl extends AbstractControl implements TankControl, Cloneable {
	...
	private static final float DEFAULT_FORWARD_SPEED = 8.0f;
	private static final float DEFAULT_BACKWARD_SPEED = 5.0f;
	...
	protected void controlUpdate(float tpf) {
		...
		if (!initialized) {
			...
			return;
                ...
                AltimeterControl ac = tank.getControl(AltimeterControl.class);
		// Get state
		...
		float deltaAngle = -steering * steeringSpeed * tpf;
                ...
                if(ac.isValid()){
                      Vector3f point = ac.getPoint();
                      deltaAltitude = 3.0f-ac.getAltitude();
                  }
		...
		tankPosition.addLocal(deltaPosition);
                tankPosition.addLocal(0.0f,deltaAltitude, 0.0f);
		angle += deltaAngle;
		...
		if (firing && lastTimeFired >= reloadTime) {
			SpawnControl projectileSpawnControl = nozzle.getControl(SpawnControl.class);
                        NozzleFlashControl fireControl = Utils.getChild((Node)spatial, "nozzleFlash", Node.class).getControl(NozzleFlashControl.class);
			if (projectileSpawnControl != null & fireControl != null) {
	...
	public void setReloadTime(float reloadTime) {
		this.reloadTime = FastMath.clamp(throttle, 0.0f, Float.MAX_VALUE);
	}
	...
	public void setThrottle(float throttle) {
		this.throttle = FastMath.clamp(throttle, -1.0f, 1.0f);
	}
