...
public class PhysicsTankControl extends AbstractControl implements TankControl, PhysicsListener, Cloneable {
	...
	private static final float DEFAULT_RELOAD_TIME = 0.0f; // sec
	//Properties
	...
	private void initialize() {
		...
		compoundCollisionShape.addChildShape(new BoxCollisionShape(new Vector3f(.5f, .1f, 3.0f)), new Vector3f(0.0f, 0.25f, 0.0f));
		compoundCollisionShape.addChildShape(new BoxCollisionShape(new Vector3f(.5f, .1f, 3.0f)), new Vector3f(0.0f, 0.25f, 0.0f));
		compoundCollisionShape.addChildShape(new BoxCollisionShape(new Vector3f(.5f, .5f, .5f)), new Vector3f(-3.5f, -0.25f, -2.5f));
		compoundCollisionShape.addChildShape(new BoxCollisionShape(new Vector3f(.5f, .5f, .5f)), new Vector3f(3.5f, -0.25f, -2.5f));
	...
	protected void controlUpdate(float tpf) {
		...
		if (firing && lastTimeFired >= reloadTime) {
			Spatial projectile;
			projectile = projectileSpawnControl.spawn("Have a nice day!");
			...
			firing = false;
			physicsFiring = true;
			listable.fireFired("Have a nice day!");
		}
		...
		if (healthObject instanceof Float) {
			...
			health -= damage;
			spatial.setUserData("health", healthObject);
			if (health <= 0.0f) {
				dead = true;
			}
			if (damage > 0.0f) {
