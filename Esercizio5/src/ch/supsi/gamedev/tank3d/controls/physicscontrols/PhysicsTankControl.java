...
public class PhysicsTankControl extends AbstractControl implements TankControl, PhysicsListener, Cloneable {
	...
	private static final float DEFAULT_TURRET_SPEED = 8.0f; // RAD/sec
	private static final float DEFAULT_CANNON_SPEED = 8.0f; // RAD/sec
	...
	private float reloadTime = DEFAULT_RELOAD_TIME; // sec
	// State
	...
	public void physicsUpdate(float tpf) {
		// --- Do stuff ---
                Vector3f tankAhead = rigidBodyControl.getPhysicsRotation().mult(Vector3f.UNIT_Z).normalize();
                Vector3f tankLeft = rigidBodyControl.getPhysicsRotation().mult(Vector3f.UNIT_X).normalize();
                
                // Compute deltas
		Vector2f deltaPosition2 = new Vector2f(throttle);
		deltaPosition2.multLocal(new Vector2f(strafeForce, throttle.getY() < 0.0f ? backwardForce : forwardForce));
		...
		deltaPosition.subtractLocal(tankLeft.mult(deltaPosition2.getX()));
                rigidBodyControl.applyForce(deltaPosition, Vector3f.ZERO);
                rigidBodyControl.applyTorque(new Vector3f(0.0f,-1*steering*turnTorque,0.0f));
                if(firing){
                    Vector3f dir = cannon.getWorldRotation().getRotationColumn(2).negate();
                    Vector3f cannonPos = cannon.getWorldTranslation();
                    Vector3f rigidCenter = rigidBodyControl.getPhysicsLocation();
                    rigidBodyControl.applyImpulse(dir.mult(cannonImpulse), cannonPos.subtract(rigidCenter));
                }
